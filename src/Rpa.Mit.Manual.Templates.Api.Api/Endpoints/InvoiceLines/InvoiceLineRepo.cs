...
namespace Rpa.Mit.Manual.Templates.Api.Api.Endpoints.Invoices
{
    ...
    internal sealed class InvoiceLineRepo : BaseData, IInvoiceLineRepo
    {
        ...
        public async Task<IEnumerable<InvoiceLineAr>> GetInvoiceLinesArByInvoiceRequestId(string invoiceRequestId, CancellationToken ct)
        {
            using (var cn = new NpgsqlConnection(await DbConn()))
            {
                if (cn.State != ConnectionState.Open)
                ...
                return await cn.QueryAsync<InvoiceLineAr>(
                            "SELECT id, value, description, fundcode, mainaccount, schemecode, marketingyear, deliverybodycode, invoicerequestid, debttype FROM invoicelines WHERE invoiceRequestId = @invoiceRequestId",
                            new { invoiceRequestId });
            }
        }
        ...
        public async Task<InvoiceLineAr> GetInvoiceLineArByInvoiceLineId(Guid invoiceLineId, CancellationToken ct)
        {
            using (var cn = new NpgsqlConnection(await DbConn()))
            {
                if (cn.State != ConnectionState.Open)
                ...
                return await cn.QuerySingleAsync<InvoiceLineAr>(
                            "SELECT id, value, description, fundcode, mainaccount, schemecode, marketingyear, deliverybodycode, invoicerequestid, debttype FROM invoicelines WHERE id = @invoiceLineId",
                            new { invoiceLineId });
            }
        }
    }
